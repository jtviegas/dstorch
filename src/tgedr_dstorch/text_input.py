...
class TextModelInput:
    ...
    def _split(
        self, x: np.ndarray, y: np.ndarray, validation_split: float = 0.2, test_split: float = 0.0
    ) -> tuple[BaseDataset, BaseDataset, BaseDataset | None]:
        ...
        Returns:
            Tuple of (train_data, validation_data, test_data) where each is a BaseDataset.
        ...
        ds_train = BaseDataset(x=train_data[0], y=train_data[1])
        ds_validation = BaseDataset(x=validation_data[0], y=validation_data[1])
        if test_data is not None:
          ds_test = BaseDataset(x=test_data[0], y=test_data[1])

        self.train = ds_train
        self.validation = ds_validation
        self.test = ds_test
